<div class="mod mod-context-menu-marker" data-connectors="markermenu,context" data-lng="" data-lat="">
	<a class="dropdown-toggle hide" data-toggle="dropdown" href="#">Dropdown trigger</a>
	<ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu">
		<li class="disabled">
			<a tabindex="-1" href="#">
				<span class="nav-header" data-id="position">Position</span>
			</a>
		</li>
		<!--REFAK: MARKER-->
		<li >
			<a tabindex="-1" href="javascript:void(0);" data-id="remove-marker">
				<i class="icon-flag"></i>
				<span>Remove marker</span>
			</a>
		</li>
		<li>
			<a tabindex="-1" href="javascript:void(0);" data-id="make-route">
				<i class="icon-resize-full"></i>
				<span>Create route from markers</span>
			</a>
		</li>
		<li>
			<a tabindex="-1" href="javascript:void(0);" data-id="calculate-distance">
				<i class="icon-screenshot"></i>
				<span>Distance from crosshair</span>
			</a>
		</li>
		<!--REFAK: MARKER END-->
	</ul>
</div>
